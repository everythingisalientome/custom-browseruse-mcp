        ...
        (function() {{
            ...
            if (params.xpath) {{
                try {{
                    ...
                    for (let i = 0; i < snapshot.snapshotLength; i++) {{
                        const el = snapshot.snapshotItem(i);
                        if (isVisible(el)) return el; // Return immediately if found
                    }}
                }} catch (e) {{
                    // Ignore invalid XPath errors and fall through to strategy 2
                }}
            }}
            ...
            // Broad selection of interactive elements
            const candidates = document.querySelectorAll(
                'input, button, a, select, textarea, [role="button"], [role="link"], [role="checkbox"], [role="radio"], [role="combobox"], [role="option"], label'
            );
            ...
            for (const el of candidates) {{
                ...
                // If text doesn't match at all, skip (unless it's an input and we matched a label tag previously)
                // Simplified: strictly require some text overlap for safety
                ...
                // A. Text Precision
                if (text === params.label) score += 100;           // Exact Match
                else if (text.startsWith(params.label)) score += 50; // Starts With
                ...
                if (params.role) {{
                    ...
                    else score -= 20; // Penalize mismatching roles
                }}

                // C. Penalize "Containers" (divs that contain the text but aren't the button)
