...
class ChromeCDP:
    ...
    def hover(self, xpath: str = None, label: str = None, role: str = None, timeout_ms=DEFAULT_TIMEOUT):
        ...
        if not point: raise RuntimeError(f"Hover failed; geometry failed: {xpath or label}")
    ...
    def double_click(self, xpath: str = None, label: str = None, role: str = None, timeout_ms=DEFAULT_TIMEOUT):
        self._ensure_page_actionable(timeout_ms=timeout_ms)
        obj_id = self._get_object_id(xpath, label, role)
        if not obj_id: raise RuntimeError(f"Double click failed; no ID for {xpath or label}")
    ...
    def fill(self, xpath: str = None, value: str = "", label: str = None, role: str = None, timeout_ms: int = DEFAULT_TIMEOUT):
        entry = self.tracer.start_step(action="fill", target=xpath or label, params={"value": value}) if self.tracer.enabled else None
        deadline = time.monotonic() + timeout_ms / 1000
        ...
        try:            
            while time.monotonic() < deadline:
                try:
                    ...
                    time.sleep(STEP_DELAY)
            raise TimeoutError(f"Fill timed out for target: {xpath or label}")
    ...
    def click(self, xpath: str = None, label: str = None, role: str = None, timeout_ms=DEFAULT_TIMEOUT):
        entry = self.tracer.start_step(action="click", target=xpath or label) if self.tracer.enabled else None
        deadline = time.monotonic() + timeout_ms / 1000
        try:
            while time.monotonic() < deadline:
                try:
                    ...
                    if entry: self.tracer.record_retry(entry)
                    time.sleep(STEP_DELAY)
            raise TimeoutError(f"Click failed: {xpath or label}")
        except Exception as e:
            if entry:
                self.tracer.failure(entry, e)
                self._capture_failure_artifacts(entry)
                ...
                for (const el of nodes) {{
                     ...
                     if (text.includes(query) || val.includes(query) || ph.includes(query) || name.includes(query) || id.includes(query) || aria.includes(query) || title.includes(query)) {{
                         ...
                         candidates.push({{
                            ...
                            xpath: xpath,
                            attributes: {{
                                id: el.id, class: el.className, title: el.getAttribute('title'), role: role, type: el.getAttribute('type'), 'aria-label': el.getAttribute('aria-label'), onclick: el.hasAttribute('onclick') ? 'true' : 'false'
                            }}
                         }});
                     }}
                }}
            ...
            while (queue.length > 0) {{
                ...
                nodes.forEach(el => {{
                    ...
                    results.push({{ 
                        ...
                        xpath: el.id ? `//*[@id='${{el.id}}']` : `//${{el.tagName}}`,
                        visible: true 
                    }});
                }});
